...
function useSummaryStats(): { stats: StatItem[]; isLoading: boolean; isError: boolean } {
  ...
  const stats = useMemo<StatItem[]>(() => {
    ...
    const thisWeek = list.filter((o) => new Date(o.created_at).getTime() >= weekAgo).length;
    const delivered = list.filter((o) => o.status === 'c').length;

    return [
      {
        label: 'إجمالي الطلبات', value: arDigits(total),
        hint: arDigits(total), icon: 'document-text-outline',
      },
      {
        label: 'هذا الأسبوع', value: arDigits(thisWeek),
        hint: arDigits(`${total > 0 ? Math.round((thisWeek / total) * 100) : 0}%`),
        icon: 'trending-up-outline', up: thisWeek > 0,
      },
      {
        label: 'تم التسليم', value: arDigits(delivered),
        hint: total > 0 ? arDigits(`${Math.round((delivered / total) * 100)}%`) : '—',
...
function SummaryCard() {
  ...
  return (
    <View style={cs.card}>
      ...
      {isLoading ? (
        <View style={cs.center}><ActivityIndicator color="#fff" /></View>
      ) : isError ? (
        <Text style={cs.error}>تعذر تحميل الإحصائيات</Text>
      ) : (
        <View style={cs.statsRow}>
          {stats.map((s, i) => (
            <View key={s.label} style={[cs.stat, i < stats.length - 1 && cs.statBorder]}>
...
const cs = StyleSheet.create({
  card: {
    marginHorizontal: 22, marginTop: 12, borderRadius: 24, padding: 18,
    backgroundColor: Palette.greenDk,
    shadowColor: '#1d3f2a', shadowOpacity: 0.32, shadowRadius: 28, shadowOffset: { width: 0, height: 12 },
  },
  headRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  eyebrow: { fontSize: 11, color: 'rgba(255,255,255,0.7)', fontFamily: Fonts.arabicMedium },
  title: { fontSize: 20, color: '#fff', fontFamily: Fonts.arabicBold, marginTop: 2, letterSpacing: -0.3 },
  live: {
    flexDirection: 'row', alignItems: 'center', gap: 6,
    paddingHorizontal: 12, paddingVertical: 6, borderRadius: 999,
    backgroundColor: 'rgba(255,255,255,0.14)', borderWidth: 0.5, borderColor: 'rgba(255,255,255,0.22)',
  },
  liveDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: '#a8e0a8' },
  liveTxt: { fontSize: 11, color: '#fff', fontFamily: Fonts.arabicBold },
  divider: { height: 1, backgroundColor: 'rgba(255,255,255,0.14)', marginTop: 14, marginHorizontal: -18 },
  ...
  statValue: { fontSize: 22, color: '#fff', fontFamily: Fonts.arabicBold, letterSpacing: -0.5, marginTop: 2 },
  statLabel: { fontSize: 11, color: 'rgba(255,255,255,0.78)', fontFamily: Fonts.arabicMedium },
  statHint: { fontSize: 10, color: 'rgba(255,255,255,0.65)', fontFamily: Fonts.arabicBold },
  statHintUp: { color: '#a8e0a8' },
  center: { paddingVertical: 28, alignItems: 'center' },
  error: { padding: 16, textAlign: 'center', color: '#ffd1c8', fontFamily: Fonts.arabicMedium, fontSize: 12 },
});
...
function ReportRow({ idx, r }: { idx: number; r: Report }) {
  ...
  return (
    <Pressable
      style={({ pressed }) => [ls.row, idx > 0 && ls.rowDivider, pressed && { opacity: 0.85 }]}
      ...
      <View style={{ flex: 1 }}>
        <View style={ls.titleRow}>
          <View style={ls.num}><Text style={ls.numTxt}>{arDigits(idx + 1)}</Text></View>
          <Text style={ls.title}>{r.title}</Text>
        </View>
        <Text style={ls.sub}>{r.sub}</Text>
      </View>
      <Ionicons name="chevron-back" size={14} color={Palette.ink} style={{ opacity: 0.55 }} />
    </Pressable>
  );
}
...
const ls = StyleSheet.create({
  ...
  title: { fontSize: 15, color: Palette.ink, fontFamily: Fonts.arabicBold, letterSpacing: -0.2, flexShrink: 1 },
  sub: { fontSize: 11, color: Palette.inkSoft, marginTop: 4, fontFamily: Fonts.arabic, lineHeight: 16 },
});
    ...
    <Screen>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 130 }}>
        ...
        <View style={out.wrap}>
          ...
          <View style={out.card}>
            {REPORTS.map((r, i) => <ReportRow key={r.title} idx={i} r={r} />)}
          </View>
        </View>
      </ScrollView>
    </Screen>
...
const out = StyleSheet.create({
  ...
  title: { fontSize: 16, color: Palette.ink, fontFamily: Fonts.arabicBold, letterSpacing: -0.3 },
